...
<@t.template user>
    <div style="text-align: center; color: white">
        ...
        <p>
            ...
            <#if otherUser.isModerator()>
                <span style="color: gold">Модератор</span>
            </#if>
        </p>
        <p>
            <#if !otherUser.isActive()>
                <span style="color: red">Пользователь заблокирован</span>
            </#if>
        </p>
        Электоронная почта: <b>${otherUser.email}</b>
        <h3>Посты:</h3>
        <#if posts??>
        <table class="table table-striped" style="text-align: start;">
            ...
            <#else>
                <h1 style="text-align: center">Пользователь еще не писал постов</h1>
            </#list>
            </table>
        </#if>
    </div>
